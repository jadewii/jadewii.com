...
// Remove existing Sequential products if they exist (clean slate)
gearContent = gearContent.replace(/,\s*\/\/ Sequential Products[\s\S]*?(?=\n\])/g, '')

// Find the position right before the closing bracket of the array
const closingBracketIndex = gearContent.lastIndexOf('\n]')

// Add Sequential products with proper formatting
const productsString = sequentialProducts.map(product => {
  return `  {
    id: '${product.id}',
    title: '${product.title}',
    brand: '${product.brand}',
...
const updatedContent = gearContent.slice(0, closingBracketIndex) + insertString + gearContent.slice(closingBracketIndex)

// Write the updated file
fs.writeFileSync(gearFilePath, updatedContent)

console.log('✅ GEAR FILE FIXED!')
console.log('📝 Added Sequential products with proper syntax')